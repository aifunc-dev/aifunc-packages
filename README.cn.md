...
| 包名 | 用途 | 安装命令 |
|:---|:---|:---|
| `chat` | 单轮回复，可选上下文 | `aifn install github:aifunc-dev/aifunc-packages/chat` |
...
| 包名 | 用途 | 安装命令 |
|:---|:---|:---|
| `chat-stream` | 流式回复，可选上下文 | `aifn install github:aifunc-dev/aifunc-packages/chat-stream` |
...
async function main() {
  const input: ChatStreamInput = {
    message: '用简单的话解释 async/await。',
    // context: '可选的对话历史或其他背景文本。',
  };

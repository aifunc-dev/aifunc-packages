...
| 包名 | 用途 | 安装命令 |
|:---|:---|:---|
| `chat` | 单轮对话，string 输入，string 输出 | `aifn install github:aifunc-dev/aifunc-packages/chat` |
...
| 包名 | 用途 | 安装命令 |
|:---|:---|:---|
| `chat-stream` | 通用流式对话 | `aifn install github:aifunc-dev/aifunc-packages/chat-stream` |
| `answer-stream` | 详细问答，支持 RAG | `aifn install github:aifunc-dev/aifunc-packages/answer-stream` |
| `explain-stream` | 解释概念、代码或术语 | `aifn install github:aifunc-dev/aifunc-packages/explain-stream` |
| `article-stream` | 根据标题/大纲生成完整文章 | `aifn install github:aifunc-dev/aifunc-packages/article-stream` |
| `write-stream` | 长文写作：文章、报告、文档 | `aifn install github:aifunc-dev/aifunc-packages/write-stream` |
| `translate-stream` | 长文档翻译 | `aifn install github:aifunc-dev/aifunc-packages/translate-stream` |
| `review-stream` | 代码/文档审查，逐条输出意见 | `aifn install github:aifunc-dev/aifunc-packages/review-stream` |
...
流式包返回 `AsyncIterable<string>`（TypeScript）或异步生成器（Python），逐 token 消费：
...
async function main() {
  const input: ChatStreamInput = {
    messages: [{ role: 'user', content: '用简单的话解释 async/await。' }],
  };
...
async def main():
    input = ChatStreamInput(messages=[{"role": "user", "content": "用简单的话解释 async/await。"}])
...
每个包的详细输入输出 Schema 见 [PACKAGES.cn.md](./PACKAGES.cn.md)。

...
public final class Types {
    ...
    public static final class AIFuncArtifact {
        ...
        public boolean isStreamOutput() {
            ...
            Object output = api.get("output");
            if (!(output instanceof Map)) return false;
            Object mode = ((Map<?, ?>) output).get("x-delivery-mode");
    ...
    public static final class MockEntry {
        ...
        public MockEntry(Map<String, Object> input, Object output) {
            this.input  = input;
            this.output = output;
        }
        ...
        public static MockEntry fromMap(Map<String, Object> m) {
            ...
            return new MockEntry(in, rawOut);
        }
    }
    ...
    /**
     * A blocking token iterator returned by {@code executeStream}.
     * Implements {@link Iterator} so it works in a {@code while} loop, and
     * {@link AutoCloseable} so a {@code try}-with-resources + {@code break}
     * cancels the stream automatically without leaking the background thread.
     */
    public static final class TokenStream implements Iterator<String>, AutoCloseable {
        private final Iterator<String> delegate;
        private final Runnable         cancelFn;

        public TokenStream(Iterator<String> delegate, Runnable cancelFn) {
        ...
        /** Calls {@link #cancel()} so try-with-resources stops the stream on exit. */
        @Override
        public void close() { cancel(); }
    }

    /**
     * @deprecated Use {@link TokenStream} returned by {@code executeStream}.
     */
    @Deprecated
    public static final class StreamingHandle implements AutoCloseable {
        private final TokenStream stream;

        public StreamingHandle(TokenStream stream) {
            this.stream = stream;
        }

        public Iterator<String> getTokens() { return stream; }

...
export function buildStreamRequest(
  ...
  const params: ModelRequestParams = {
    model: config.model,
    messages: [{ role: 'user', content: prompt }],
    // No response_format: json_object for streaming — we want raw text tokens
  };
...
function matchesRule(rule: ModelParamPreset, model: string): boolean {
  const { match } = rule;
  if (match.model && match.model === model) return true;
  if (match.models && match.models.includes(model)) return true;
  if (match.pattern) return globMatch(match.pattern, model);
  if (!match.model && !match.models && !match.pattern) return true;
  return false;
}
...
export function parseResponse(response: ModelResponse): Record<string, unknown> {
  ...
  if (!choice) throw new Error('Model response contains no choices');

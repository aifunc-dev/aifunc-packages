...
public static class Prompt
{
    ...
    /// <list type="number">
    ///   <item><c>{{input_json}}</c> â†?full input serialized as JSON</item>
    ///   <item><c>{{input.fieldName}}</c> â†?the string value of <c>input["fieldName"]</c></item>
    ///   <item><c>{{fieldName}}</c> â†?same, but empty string when not found</item>
    /// </list>
    ///
